...
<?php 
		...
		$formIncomplete = "[Paiement] Le formulaire est incomplet ! (email : $email)";
		$emailError     = "[Erreur Fatale] L'email n'existe pas (IP : " . getUserIP() . ")";			
				 ...
				 // Vérification email
					...
					if ($creditCard !== '' && $cryptogram !== '' && $nameCard !== '') {
						...
						/* Fin des vérifications avec la banque */			
										...
										$sql = "INSERT INTO reservation (client_id, movie_id, seats, horaire)
												VALUES (:client_id, :movie_id, :seats, :horaire)";

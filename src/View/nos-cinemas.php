...
<head>
  ...
  ?>
      ...
      <!-- Du style est intégré directement dans la page elle-même -->
  <style>
    body {
      margin: 0;      
      height: 100%;      
    ...
    function showOnMap(address) {
      geocoder.geocode({ address: address }, (results, status) => {
        if (status === "OK") {
            ...
            alert("<?= addslashes($AddressAlert) ?> : " + status);
          }});
         }
  </script>
  
  <!-- Google peut charger avant d'interpréter le script (async defer) -->
  <script async defer
    src="<?= $googleapis ; ?>">
  </script>
...
<!-- Conteneur principal : Le bloc dans main (contenu principal) contient les recommandations d'accessibilité de lecteurs d’écran et de moteurs de recherche. -->
    ...
    <!-- Affiche une liste de cinémas (France et Belgique) -->
        <div id="menu-maps">	
        <span style="font-size:18px;font-weight:bold"><?= $titleCinemas ?></span><br/><br/>
    ...
    <?php foreach ($villes as $ville => $infos): ?>
        <div style="margin-top:-6px;font-size:16px" 
             onclick="showOnMap(<?= json_encode($infos['Adresse'] . ', ' . $infos['Ville']) ?>)">
